...
function assessment() {
 ...
 const submit=node("button","Corrigir");submit.type="submit";const result=node("p",undefined,"course-status");form.append(submit,result);form.onsubmit=(event)=>{event.preventDefault();let hits=0;const missed=[];course.assessment.forEach((q,i)=>{const checked=form.querySelector(`input[name=q${i}]:checked`);if(checked){draft.answers[i]=Number(checked.value);if(draft.answers[i]===q.answer)hits++;else missed.push(q.module)}else missed.push(q.module)});save();result.textContent=`${hits}/10 acertos. ${hits>=7?"Referência de domínio atingida. Guarde suas evidências e seu plano.":"Revise os módulos: "+[...new Set(missed)].join(", ")+"."} A validação oficial dependerá da área de alunos.`};panel.append(form);highlight(-1);panel.focus();
}
